...
function dialogAjaxDone(json) {
    DWZ.ajaxDone(json);
    if (json[DWZ.keys.statusCode] == DWZ.statusCode.ok || json[DWZ.keys.statusCode] == DWZ.statusCode.warn) {
        if (json.navTabId) {
            //判断是否是 dialog 弹窗修改
...
function dwzPageBreak(options) {
    ...
    try {
        ...
        var $formT = $("form[rel='pagerForm']", $parent);
        if ($formT.length == 1) {
            var sname = "";
            var control;
            $form.find("input[type='hidden']").each(function () {
                sname = $(this).prop('name');
                control = $formT.find("input[id='" + sname + "']");
                if (control.length != 1)
                    control = $formT.find("select[id='" + sname + "']");
...
$.fn.extend({
    ...
    dwzExport: function () {
        function _doExport($this) {
            ...
            try {
                //导出时，查询条件赋值
                var $formT = $("form[rel='pagerForm']", $p);
                if ($formT.length == 1) {
                    var sname = "";
                    var control;
                    $form.find("input[type='hidden']").each(function () {
