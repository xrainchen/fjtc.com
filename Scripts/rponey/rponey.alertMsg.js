...
var alertMsg = {
	...
	_keydownOk: function (event) {
		if (event.keyCode === rponey.keyCode.ENTER) event.data.target.trigger("click");
		return false;
	},
	_keydownEsc: function (event) {
		if (event.keyCode === rponey.keyCode.ESC) event.data.target.trigger("click");
	},
	/**
	 * 
	 * @param {Object} type
	...
	_open: function (type, msg, buttons) {
		...
		if (buttons) {
			for (var i = 0; i < buttons.length; i++) {
				...
				butsHtml += rponey.frag["alertButFrag"].replace("#butMsg#", buttons[i].name).replace("#callback#", sRel);
			}
		}
		var boxHtml = rponey.frag["alertBoxFrag"].replace("#type#", type).replace("#title#", this._getTitle(type)).replace("#message#", msg).replace("#butFragment#", butsHtml);
		...
		for (var i = 0; i < buttons.length; i++) {
			...
			if (buttons[i].keyCode === rponey.keyCode.ENTER) {
				jDoc.bind("keydown", { target: jButs.eq(i) }, this._keydownOk);
			}
			if (buttons[i].keyCode === rponey.keyCode.ESC) {
	...
	_alert: function (type, msg, options) {
	    ...
	    $.extend(op, options);
	    var buttons = [
	    	{ name: op.okName, call: op.okCall, keyCode: rponey.keyCode.ENTER }
	    ];
	...
	confirm: function (msg, options) {
		...
		var buttons = [
			{ name: op.okName, call: op.okCall, keyCode: rponey.keyCode.ENTER },
			{ name: op.cancelName, call: op.cancelCall, keyCode: rponey.keyCode.ESC }
		];
